...
SmallParticle::SmallParticle(const std::string& name) :
	BaseSystem(name),
	m_sx(1.0f),
	m_sy(1.0f),
	m_sz(1.0f),
	m_mass(1.0f)
{
	...
	setVector(initial_pos, 0, 0, 0);
	setVector(initial_vel, cos(45), sin(45), cos(std::rand() % 360));

}	// SmallParticle

void SmallParticle::getState(double* p, double* v)
{

	VecCopy(p, m_pos);
	VecCopy(v, m_vel);

}	// SmallParticle::getState

void SmallParticle::setState(double* p)
{

	VecCopy(m_pos, p);

}	// SmallParticle::setState

void SmallParticle::reset(double time)
{
...
int SmallParticle::command(int argc, myCONST_SPEC char** argv)
{
	if (argc < 1)
	{
...
void SmallParticle::display(GLenum mode)
{
