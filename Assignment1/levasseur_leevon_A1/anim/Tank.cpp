...
Tank::Tank(const std::string& name, char* modelFile, HermiteSpline* givenTankPath) :
	BaseSystem(name),
	m_sx(3.0f),
	m_sy(3.0f),
	m_sz(3.0f)
...
void Tank::reset(double time)
{
	if (needsInit)
	{
		// Have not initialized tankpath points, default to 0,0,0
		setVector(m_pos, 0.0, 0.0, 0.0);
	}
	setState(tankpath->getPoints()[0]);
...
void Tank::display(GLenum mode)
{
	...
	if (needsInit)
	{
		// tankpath has been loaded so getPoints()[0] will return the first point.
		setState(tankpath->getPoints()[0]);
		needsInit = False;
	}
