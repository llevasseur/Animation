...
SampleForceSimulator::SampleForceSimulator(const std::string& name, BaseSystem* target) :
	BaseSimulator(name),
	m_object(target)
{
}	// SampleGravitySimulator

SampleForceSimulator::~SampleForceSimulator()
{
}	// SampleGravitySimulator::~SampleGravitySimulator

int SampleForceSimulator::step(double time)
{
	double g = -1.8;
	Vector pos;
